...
use raft_proto::rpc::{PutRequest, PutResponse, GetRequest, GetResponse};
...
impl KeyValueService for RaftKvService {
    async fn put(&self, request: Request<PutRequest>) -> Result<Response<PutResponse>, Status> {
        ...
        // Send to RaftDriver
        if let Err(_) = self.cmd_tx.send(ServerCommand::Rpc(cmd)) {
                ...
                if let Some(id_str) = e.strip_prefix("Not leader: ") {
                    if let Ok(leader_id) = id_str.trim().parse::<u64>() {
                        if let Some(addr) = self.peers_map.get(&leader_id) {
                             status.metadata_mut().insert("x-raft-leader-id", leader_id.to_string().parse().unwrap());
                             status.metadata_mut().insert("x-raft-leader-addr", addr.to_string().parse().unwrap());
                        }
                    }
                }
    ...
    async fn get(&self, request: Request<GetRequest>) -> Result<Response<GetResponse>, Status> {
        ...
        match rx.await {
            Ok(Some(val)) => Ok(Response::new(GetResponse { value: val, found: true })),
            Ok(None) => Ok(Response::new(GetResponse { value: "".into(), found: false })),
            Err(_) => Err(Status::internal("Raft driver dropped callback")),
        }
    }
}

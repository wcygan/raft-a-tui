...
pub struct RaftNode {
    ...
    callbacks: HashMap<Vec<u8>, tokio::sync::oneshot::Sender<Result<(), String>>>,
    logger: Logger,
}
...
impl RaftNode {
    ...
    /// * `callback` - Optional callback to be notified when committed
    ///
    /// # Errors
    ...
    pub fn take_callback(&mut self, context: &[u8]) -> Option<tokio::sync::oneshot::Sender<Result<(), String>>> {
